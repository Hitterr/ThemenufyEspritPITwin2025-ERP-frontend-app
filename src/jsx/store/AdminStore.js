...
const useAdminStore = create((set) => ({
	...
	addAdmin: async (adminData) => {
		try {
			const { data } = await apiRequest.post(API_URL, adminData);
			set((state) => ({
	...
	updateAdmin: async (id, adminData) => {
		try {
			const { data } = await apiRequest.put(`${API_URL}/${id}`, adminData);
			set((state) => ({
	...
	archiveAdmin: async (id) => {
		try {
			await apiRequest.patch(`${API_URL}/${id}/archive`);
			set((state) => ({
	...
	deleteAdmin: async (id) => {
		try {
			await apiRequest.delete(`${API_URL}/${id}/delete`); // Updated endpoint
			set((state) => ({
	...
	toggleBlock: async (id, currentBlocked) => {
		try {
			const { data } = await apiRequest.put(`${API_URL}/${id}`, {
