	...
	const validateStep = (step) => {
		...
		switch (step) {
			...
			case 1:
				...
				if (!formData.telephone.match(/^\+?[1-9]\d{6,14}$/))
					newErrors.telephone = "Numéro de téléphone invalide";
				else if (!phoneRegex.test(formData.telephone))
					newErrors.telephone = "Numéro invalide (8 à 15 chiffres).";
				break;
			case 2:
			...
			case 3:
				...
				if (!formData.adresseRestaurant)
					newErrors.adresseRestaurant = "L'adresse est requise.";
				if (!formData.cuisineType)
					newErrors.cuisineType = "Le type de cuisine est requise.";
				if (!formData.payCashMethod)
					newErrors.payCashMethod = "Veuillez sélectionner la méthode de paiment .";
				if (!formData.promotion) newErrors.promotion = "la promotion est requise .";
				break;
			case 4:
				if (!recaptchaValue)
					newErrors.recaptcha = "Veuillez vérifier que vous n'êtes pas un robot.";
				break;
			default:
				break;
		}
							...
							<form className="col-12">
								...
								>
									<div className="card shadow-lg p-4 w-100">
										...
										<div className="d-flex justify-content-center my-4 p-5">
											<Stepper activeStep={goSteps} alternativeLabel>
												<Step
													icon={<FaUser sx={{ fontSize: "2rem" }} />}
													label="Informations Personnelles"
												/>
												<Step
													icon={<FaEnvelope sx={{ fontSize: "2rem" }} />}
													label="Contact"
												/>
												<Step
													icon={<FaLock sx={{ fontSize: "2rem" }} />}
													label="Sécurité"
												/>
												<Step
													icon={<FaLock sx={{ fontSize: "2rem" }} />}
												...
												/>
												<Step
													icon={<FaCheckCircle sx={{ fontSize: "2rem" }} />}
													label="Confirmation"
												/>
											</Stepper>
										</div>
										{goSteps === 0 && (
											...
											>
												...
												{errors.datedenaissance && (
													<p className="text-danger">{errors.datedenaissance}</p>
												)}
												<select
													name="pays"
													className="form-control mb-3"
										...
										{goSteps === 3 && (
											...
											>
												...
												{errors.payCashMethod && (
													<p className="text-danger">{errors.payCashMethod}</p>
												)}
												<select
													name="promotion"
													className="form-control mb-3"
												...
												{errors.promotion && (
													<p className="text-danger">{errors.promotion}</p>
												)}
												<button className="btn btn-secondary mt-3" onClick={handlePrev}>
													Précédent
												</button>
