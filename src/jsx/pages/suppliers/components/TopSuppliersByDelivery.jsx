...
import { BarChart, Bar, Tooltip, ResponsiveContainer, XAxis, YAxis, Cell, Legend } from "recharts";
import { Spinner, Alert, Badge, Button, Modal, Table, Container } from "react-bootstrap";
...
const TopSuppliersByDelivery = () => {
  ...
  const fetchSuppliers = async () => {
    ...
    try {
      ...
      const chartData = stats
        .map((s) => {
          const avgDeliveryTime = parseFloat(s.averageDeliveryTimeDays.toFixed(1));
          const targetDeliveryTime = 5;
          const onTimePercentage =
            avgDeliveryTime <= targetDeliveryTime
              ? 100
              : Math.max(0, 100 - ((avgDeliveryTime - targetDeliveryTime) / targetDeliveryTime) * 100);
          return {
            name: s.supplierName,
      ...
      setError(err.message || "Failed to fetch supplier data. Please try again.");
  ...
  const getBarColor = (rank) =>
    rank === 1 ? "#FFD700" : rank === 2 ? "#C0C0C0" : rank === 3 ? "#CD7F32" : "#8884d8";
  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div className="custom-tooltip p-3 bg-white border rounded shadow-sm">
        ...
        <p className="mb-1">
          <strong>Delivery:</strong> <Badge bg="primary">{data.value} days</Badge>
        </p>
        <p className="mb-1">
        ...
        <p className="mb-1">
          <strong>Orders:</strong> <Badge bg="secondary">{data.totalDeliveries}</Badge>
        </p>
      </div>
    );
  };
  ...
  return (
    <Container fluid className="px-0 d-flex justify-content-center" style={{ minHeight: "100vh", backgroundColor: "#f8f9fa" }}>
      <style>
        {`
          .main-card {
            width: 100%;
          ...
          .header-title {
            max-width: 70%; /* Prevent title from overlapping buttons */
          }
          .card-body {
            flex: 1;
            display: flex;
      ...
      <div className="main-card">
        <div className="card-header">
          ...
          <div className="header-buttons">
            <Button variant="outline-primary" size="sm" onClick={() => setShowRanking(true)} className="me-2">
              <FiList className="me-1" /> View Full Ranking
            </Button>
            <Button variant="outline-primary" size="sm" onClick={fetchSuppliers} disabled={loading}>
        ...
        <div className="card-body">
          ...
          {error && (
            <Alert variant="danger" className="mt-3">
              {error}
              <Button variant="outline-danger" size="sm" className="ms-2" onClick={fetchSuppliers}>
                Retry
              </Button>
            </Alert>
          )}
            ...
            <>
              <div className="chart-container">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={suppliersData} layout="vertical" margin={{ top: 20, right: 30, left: 10, bottom: 60 }}>
                    <XAxis type="number" axisLine={{ stroke: "#ddd" }} tickLine={{ stroke: "#ddd" }} />
                    <YAxis dataKey="name" type="category" width={0} axisLine={{ stroke: "transparent" }} tickLine={{ stroke: "transparent" }} />
                ...
                <div className="row">
                  {suppliersData.slice(0, 3).map((supplier, index) => (
                    <div key={index} className="col-md-4 mb-3">
                      <div className={`card h-100 border-0 shadow-sm top-performer-card ${index === 0 ? "border-warning border-2" : ""}`}>
                        <div className="card-body text-center">
                          <Badge pill bg={index === 0 ? "warning" : index === 1 ? "secondary" : "info"} className="mb-2">
                          ...
                          <p className="mb-0">
                            ...
                            <Badge bg="secondary">{supplier.totalDeliveries}</Badge>
                          </p>
                        </div>
                      </div>
                    </div>
        ...
        <Modal show={showRanking} onHide={() => setShowRanking(false)} size="xl" centered className="modal-fullscreen-lg-down">
          <Modal.Header closeButton className="border-bottom-0">
            <Modal.Title className="d-flex align-items-center">
              <FiAward className="me-2 text-warning" /> Supplier Performance Ranking
            </Modal.Title>
          </Modal.Header>
                ...
                <tbody>
                  {suppliersData.map((supplier) => (
                    <tr key={supplier.rank}>
                      <td>
                        <Badge pill bg={supplier.rank <= 3 ? ["warning", "secondary", "info"][supplier.rank - 1] : "light"} text="dark">
                          #{supplier.rank}
                        </Badge>
                      </td>
