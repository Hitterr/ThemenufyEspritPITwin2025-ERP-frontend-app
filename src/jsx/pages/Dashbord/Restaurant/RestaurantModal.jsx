...
export default function RestaurantModal({
  ...
  useEffect(() => {
    if (
      ...
      const selected =
        modalState[
          type === "restaurant" ? "selectedRestaurant" : "selectedAdmin"
        ];
      setFormData(
        type === "restaurant"
          ? {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "blocked" || name === "isEmailVerified"
          ? value === "true"
          : value,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      swal(
        "Success",
        modalState.isEditMode ? `${type} updated` : `${type} added`,
        "success"
      );
          ...
          modalState[
            ...
            <div>
              {type === "restaurant" ? (
                <>
                  <p>
                    <strong>Name:</strong> {modalState.selectedRestaurant.name}
                  </p>
                  <p>
                    <strong>Address:</strong>{" "}
                    {modalState.selectedRestaurant.address}
                  </p>
                  <p>
                    <strong>Phone:</strong>{" "}
                    {modalState.selectedRestaurant.phone}
                  </p>
                </>
              ) : (
                <>
                  <p>
                    <strong>Email:</strong> {modalState.selectedAdmin.email}
                  </p>
                  <p>
                    <strong>Verified:</strong>{" "}
                    {modalState.selectedAdmin.isEmailVerified ? "Yes" : "No"}
                  </p>
                  <p>
                    <strong>Blocked:</strong>{" "}
                    {modalState.selectedAdmin.blocked ? "Yes" : "No"}
                  </p>
                </>
              )}
            </div>
